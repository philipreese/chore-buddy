    ...
    IRecipient<ChoreActivatedMessage>
{
    ...
    private async Task DeleteChore(ChoreDisplayItem chore)
    {
        ...
        if (confirm)
        {
            ...
            await Snackbar.Make("Chore deleted", duration: TimeSpan.FromSeconds(1)).Show();
            await LoadData();
        }
    }
    ...
    private async Task DeleteAllChores()
    {
        ...
        if (confirm)
        {
            ...
            await Snackbar.Make("All chores deleted", duration: TimeSpan.FromSeconds(1)).Show();
            await LoadData();
        }
    }
    ...
    private async Task ArchiveChore(ChoreDisplayItem item)
    {
        ...
        if (confirm)
        {
            ...
            await Snackbar.Make("Chore archived", duration: TimeSpan.FromSeconds(1)).Show();
            await LoadData();
        }
    }
