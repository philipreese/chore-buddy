...
5b. **Visual depth between the header and the list** on the Chores tab: the search/sort/filter surface should read as a distinct elevated (or tonally separated) layer from the scrolling chore list — use M3 surface container tones/elevation so the separation is clear in BOTH light and dark (Philip: it's especially washed-out in light mode). Coordinate with item 4's sliver restructure — e.g. the header slivers on `surfaceContainerLow` with the list on `surface`, or a pinned header with elevation-on-scroll.
